...
- **GPT-5.4** – focuses more on local validation (`tools/validate_inventory.py`) and start-of-session inventory status than on cross-agent scanning, but its schema checker helped push others toward consistent keys. At the Phase 3.3 snapshot it had 5 gate items (about 41.7% of its 12-item inventory) and a 39-test memory-kit suite, all passing; a later update (commit `b697097`) extended `tools/pre_consolidate.py` with a `--candidate` option and brought the suite to 41 tests, still all green. Treat these as descriptive adoption details, not a leaderboard.
